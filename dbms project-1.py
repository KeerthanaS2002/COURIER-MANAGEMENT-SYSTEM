...
def track_courier():
    ...
    row = cur.fetchone()
    if row:
        status = (f"Courier ID: {row[0]}\nStatus: {row[1]}\n"
                  f"Sender: {row[2]} {row[3]}, {row[4]}\n"
                  f"Receiver: {row[5]}, {row[6]}, {row[7]}")
...
def insert_courier():
    ...
    messagebox.showinfo('Success', f'Courier added successfully with ID: {courier_id}')

# Function to update courier status
def update_status():
    cur.execute("""
        UPDATE couriers SET status = %s WHERE courier_id = %s
    """, (update_status_var.get(), update_courier_id.get()))
    
    mydb.commit()
    messagebox.showinfo('Success', f'Courier ID: {update_courier_id.get()} status updated to {update_status_var.get()}')
...
# Main Page
def main_page():
    global master, rec_name, rec_address, rec_phone, track_courier_id, update_courier_id, update_status_var
    master = Tk()
    ...
    Label(master, text="Update Courier ID").pack()
    Entry(master, textvariable=update_courier_id).pack()
    Label(master, text="Update Status").pack()
    Entry(master, textvariable=update_status_var).pack()
    Button(master, text="Update Status", command=update_status).pack()
